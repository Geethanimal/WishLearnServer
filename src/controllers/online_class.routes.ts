...
online_classRouter.post("/:id/joinLiveClass", async (req, res) => {
    try {
        const classid = req.body;
        const studentid = req.params.id;
        const query = { _id: new mongodb.ObjectId(classid) };
        const result = await collections.online_classes.updateOne(query,{$push: {students:studentid}});
...
// 
 online_classRouter.put("/joinclass/:id", async (req, res) => {
    try {
        const id = req?.params?.id;
        let student:string = req.body.students[0];
        const query = { _id: new mongodb.ObjectId(id) };
        console.log(student);

        // const result = await collections.online_classes.updateOne(query, { $push: {students:online_class.students} });
